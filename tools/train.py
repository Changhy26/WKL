...
def main(cfg, resume, opts):
    ...
    if cfg.LOG.WANDB:
        try:
            ...
            # "my-first-sweep"  cfg.EXPERIMENT.PROJECT
            wandb.init(project=cfg.EXPERIMENT.PROJECT+"_sweep", name=experiment_name, tags=tags, dir='wandb')

            # sweep
            cfg.WKD.LOSS.WKD_LOGIT_WEIGHT = wandb.config["WKD.LOSS.WKD_LOGIT_WEIGHT"]  # WKD权重
            cfg.WKD.COST_MATRIX_SHARPEN = wandb.config["WKD.COST_MATRIX_SHARPEN"]  # sharpen系数
            cfg.WKD.COST_MATRIX_SHARPEN = wandb.config["WKD.TEMPERATURE"]  # 温度
            cfg.WKD.LOSS.KL_WEIGHT = wandb.config["WKD.LOSS.KL_WEIGHT"] # KL权重
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", type=str, default="path/to/configs/cifar100/wkd_f/res32x4_res8x4.yaml")
