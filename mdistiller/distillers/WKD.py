...
def kl_feature_loss(f_s, f_t, eps):
    f_s_avg, f_t_avg = f_s.mean(dim=(-1, -2)), f_t.mean(dim=(-1, -2))
    f_s_std, f_t_std = torch.sqrt(f_s.var(dim=(-1, -2)) + eps), torch.sqrt(f_t.var(dim=(-1, -2)) + eps)
    # f_s_std, f_t_std = f_s.var(dim=(-1, -2)), f_t.var(dim=(-1, -2))
...
class WKD(Distiller):
    """This is the implementation of WKL, which combines WKD and KL through a decay function to achieve stage-adaptive hybrid distillation loss"""
    ...
    def __init__(self, student, teacher, cfg):
        ...
        # ablation for kl, dkd, wkd, dwkd, wkd_kl, dkwd_dkd
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if self.enable_wkdl:
            ...
            # kl,dkd,tckd,nckd,  wkd,dwkd,twkd,nwkd
            losses_vis = {
                "loss_kl": loss_kl,
                "loss_dkd": loss_dkd,
                "loss_tckd": loss_tckd,
            ...
            if self.distiller_name == "kl":
                ...
                # g_t = 0.5 * (1 + math.cos((kwargs['epoch'] - decay_start_epoch)/(self.cfg.SOLVER.EPOCHS - decay_start_epoch) * math.pi))
